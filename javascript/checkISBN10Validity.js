...
Return true if (and only if) isbn is a valid 10-digit ISBN.
*/
function validISBN10(isbn) {
  let ans = false
  const isbnRegex = new RegExp(/^[0-9]{9}[0-9X]$/g);
...
function validISBN10Extra(isbn) {
  let ans = false
  const isbnRegex = new RegExp(/^[0-9]{9}[0-9X]$/g);
  if (isbnRegex.test(isbn)) {
    let sum = 0
    ...
    sum += (isbn.length) * (isbn[isbn.length-1] === "X" ? 10 : Number(isbn[isbn.length-1]))
    ans = sum % 11 === 0
  }
  return ans
}