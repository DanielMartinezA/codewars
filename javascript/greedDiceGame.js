...
Your objective is to score a throw according to these rules.
...
function score(dice) {
  let score = 0
  let valuesObj = {};
  dice.forEach(elem => elem in valuesObj ? valuesObj[elem]++ : valuesObj[elem] = 1);
  if (valuesObj[1] >= 3) {
    score += 1000
    valuesObj[1] -= 3
  }
  ...
  if (1 in valuesObj) {
    score += 100*valuesObj[1]
  }
  if (5 in valuesObj) {
    score += 50*valuesObj[5]
  }
  return score
}

/* Solution from other people */
function score1(dice) {
  var dc = [0,0,0,0,0,0];
