...
Unfortunately he is not good at math.
...
function cakes(recipe, available) {
  let keysArr = Object.keys(recipe)
  let min = Math.trunc( (available[keysArr[0]] || 0) / recipe[keysArr[0]] )
  for (let i = 1; i < keysArr.length; i++) {
    min = Math.min(min, Math.trunc( (available[keysArr[i]] || 0) / recipe[keysArr[i]] ))
  }
  return min
}
